...
static CFStringRef tasklib_cfstring_from_bytes(const char* bytes, size_t len) {
	if (len == 0) {
...
static int tasklib_keychain_get(
	const char* service,
	size_t serviceLen,
	const char* account,
	size_t accountLen,
	...
	CFStringRef serviceRef = tasklib_cfstring_from_bytes(service, serviceLen);
	CFStringRef accountRef = tasklib_cfstring_from_bytes(account, accountLen);
...
static int tasklib_keychain_set(
	const char* service,
	size_t serviceLen,
	const char* account,
	size_t accountLen,
...
const keychainServicePrefix = "com.tasklib.keychain"
